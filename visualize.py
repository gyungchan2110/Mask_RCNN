...
def display_instances(image, result, class_names, filename,
    ...
    #     masked_image = np.expand_dims(masked_image, -1)    
    for i in range(N):
        color = colors[i]
        ...
        for verts in contours:
            ...
            p = Polygon(verts, linewidth = 2, facecolor=color, edgecolor=color,alpha = 0.5)
            ax.add_patch(p)
            # p = Polygon(verts, linewidth = 0, facecolor=color, edgecolor="none", alpha = 0.5)
            # ax.add_patch(p)
    ...
    if truemask is not None:    
        ...
        padded_mask[1:-1, 1:-1] = truemask[:,:,0]
        contours = find_contours(padded_mask, 0.5)
        for verts in contours:
            # Subtract the padding and flip (y, x) to (x, y)
            verts = np.fliplr(verts) - 1
